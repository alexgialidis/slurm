/*****************************************************************************\
 *  alex.c - Simple alex (FIFO) scheduler plugin.
...
/*********************** local variables *********************/
static bool stop_alex = false;
static pthread_mutex_t term_lock = PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t  term_cond = PTHREAD_COND_INITIALIZER;
static bool config_flag = false;
static int alex_interval = BACKFILL_INTERVAL;
...
/* Terminate alex_agent */
extern void stop_alex_agent(void)
{
...
static void _load_config(void)
{
	...
	if (sched_params && (tmp_ptr=strstr(sched_params, "interval=")))
		alex_interval = atoi(tmp_ptr + 9);
	if (alex_interval < 1) {
		error("Invalid SchedulerParameters interval: %d",
		      alex_interval);
		alex_interval = BACKFILL_INTERVAL;
	}
...
static void _compute_start_times(void)
{
	...
	while ((job_queue_rec = (job_queue_rec_t *) list_pop(job_queue))) {
		...
		if (job_ptr->details->min_nodes == 2) {
			continue;
		}
...
extern void alex_reconfig(void)
{
	config_flag = true;
}

/* alex_agent - detached thread periodically when pending jobs can start */
extern void *alex_agent(void *args)
{
	time_t now;
	double wait_time;
